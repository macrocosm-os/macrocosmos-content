...
4. Multi-step reasoning - enhanced inference task which requires miners to match o1-style reasoning behaviour by either fine-tuning or using agentic workflows.
5. Programming - code completion using SOTA code models.
...
Our incentive mechanism is based on the similarity between miner completions and the validators’ own reference answer. High quality reference answers are thus of utmost importance, as they set the target for the miners. The validators currently use Llama3 70B [\[Code Reference\]](https://github.com/macrocosm-os/prompting/blob/00d67a40732b831ac26c3bfc644b5ace7655f22b/prompting/utils/config.py#L287), which is of comparable quality to GPT-3.5 Turbo. To push miners to create models that outperform even Llama 70B, we use RAG generation to generate the reference answers, which miners need to match. This creates a power imbalance, where miners have to continue innovating in order to be able to match the validator’s references.

For brevity, we will describe the incentive mechanism for a single task: web question answering. For further information we refer the reader to [SN1\_Validation.md](https://github.com/macrocosm-os/prompting/blob/main/docs/SN1_validation.md).

* Validator pulls a random webpage indexed by DDG [\[Code Reference\]](https://github.com/macrocosm-os/prompting/blob/6ac1e78690af8ee1d895bc6f317cc8df6ed4fbc5/prompting/datasets/random_website.py#L24-L38).
* Validator LLM generates “query” that can be objectively answered using the article contents [\[Code Reference\]](https://github.com/macrocosm-os/prompting/blob/6ac1e78690af8ee1d895bc6f317cc8df6ed4fbc5/prompting/tasks/qa.py#L21-L28).
* Validator LLM generates a “reference” answer to the query with the benefit of the wikipedia article, ensuring a high quality answer [\[Code Reference\]](https://github.com/macrocosm-os/prompting/blob/6ac1e78690af8ee1d895bc6f317cc8df6ed4fbc5/prompting/tasks/qa.py#L31-L39).
* Validator sends “challenge” to miners and collects streamed responses [\[Code Reference\]](https://github.com/macrocosm-os/prompting/blob/6ac1e78690af8ee1d895bc6f317cc8df6ed4fbc5/prompting/tasks/task_sending.py#L42-L87).
* Validator rewards completions based on how similar their completions are to the reference answer, using a combination of semantic similarity and ROUGE, and how dissimilar they are to the challenge to avoid miners simply repeating the challenge [\[Code Reference\]](https://github.com/macrocosm-os/prompting/blob/6ac1e78690af8ee1d895bc6f317cc8df6ed4fbc5/prompting/tasks/qa.py#L42-L47).
...
[Reward model in GitHub](https://github.com/macrocosm-os/prompting/tree/main/prompting/rewards)
